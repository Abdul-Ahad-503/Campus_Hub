...
class MyLostItemsScreen extends StatelessWidget {
  ...
  Widget _buildItemCard(
    ...
    final dateLost = data['dateLost'];

    String dateString = 'Unknown Date';
    if (dateLost != null) {
      try {
        if (dateLost.toString().contains('Timestamp')) {
          final timestamp = dateLost as Timestamp;
          final date = timestamp.toDate();
          dateString = '${date.day}/${date.month}/${date.year}';
        } else {
          dateString = dateLost.toString();
        }
      } catch (e) {
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => ItemDetailScreen(item: data, isLost: true),
          ),
        );
      },
      child: Container(
